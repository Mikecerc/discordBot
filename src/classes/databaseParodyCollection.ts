...
export default class DatabaseParodyCollection<k, v> extends Collection<k, v> {
    ...
    public async save() {
        ...
        let positiveDiff = this.previousCachedData.merge(
            temp,
            (_) => ({ keep: false }),
            (y) => ({ keep: true, value: y }),
            () => ({ keep: false }),
        ...
        let negativeDiff = this.previousCachedData.merge(
            temp,
            (x) => ({ keep: true, value: x }),
            (_) => ({ keep: false }),
            () => ({ keep: false }),
        ...
        let changes = this.previousCachedData.merge(
            temp,
            (_) => ({ keep: false }),
            (_) => ({ keep: false }),
    ...
    private async updateDocuments(
        positiveDiff: Collection<k, v>,
        negativeDiff: Collection<k, v>,
        changes: Collection<k, v | undefined>,
    ): Promise<boolean> {
        if (positiveDiff.size > 0) {
            await this.model
                .insertMany(
                    positiveDiff.map((value, key) => ({
                        key: key,
                        value: value,
                    })),
                )
        ...
        if (negativeDiff.size > 0) {
            ...
            await this.model
                .deleteMany({ key: { $in: negativeDiff.map((_, k) => k) } })
                .catch((error) => {
                    throw new Error(error);
                });
        }
        ...
        if (changes != undefined && changes.size > 0) {
            ...
            await this.model.bulkWrite(bulkOps).catch((error) => {
                throw new Error(error);
            });
        }
    ...
    public async loadDocuments(): Promise<HydratedDocument<IRecord<k, v>>[]> {
        ...
        return await this.model.find({}).catch((error) => {
            throw new Error(error);
        });
    }
}
