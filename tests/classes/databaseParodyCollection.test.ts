...
describe('DatabaseParodyCollection', function () {
    ...
    it('should add items and save to the database', async function () {
        const dbCollection = new DatabaseParodyCollection<string, any>('testmodel');
        ...
        expect(models.map((model: any) => ({ key: model.key, value: model.value }))).to.deep.equal([{ key: 'key1', value: 'value1' }, { key: 'key2', value: 'value2' }]);
        dbCollection.clear()
        await dbCollection.save();
    });
    ...
    it('should remove items and save to the database', async function () {
        ...
        expect(models1.map((model: any) => ({ key: model.key, value: model.value })).sort()).to.deep.equal([{ key: 'key1', value: 'value1' }, { key: 'key2', value: 'value2' }]);

        dbCollection.delete('key1');
        await dbCollection.save();

        const models2 = await dbCollection.loadDocuments();
        expect(models2.map((model: any) => ({ key: model.key, value: model.value })).sort()).to.deep.equal([{ key: 'key2', value: 'value2' }]);
        dbCollection.clear();
        await dbCollection.save();
    });
    ...
    it('should modify items and save to the database', async function () {
        ...
        const models1 = await dbCollection.loadDocuments();
        expect(models1.map((model: any) => ({ key: model.key, value: model.value }))[0]).to.deep.equal({ key: 'key1', value: 'value1' });

        dbCollection.set('key1', 'newValue1');
        await dbCollection.save();

        const models2 = await dbCollection.loadDocuments();
        expect(models2.map((model: any) => ({ key: model.key, value: model.value }))).to.deep.equal([{ key: 'key1', value: 'newValue1' }]);
    ...
    it('should handle combinations of add, remove, and modify before save', async function () {
        ...
        await dbCollection.save();
        const models1 = await dbCollection.loadDocuments();
        expect(models1.map((model: any) => ({ key: model.key, value: model.value })).sort())
            .to.deep.equal([{ key: 'key1', value: 'value1' }, { key: 'key2', value: 'value2' }, { key: 'key3', value: 'value3' }]);
        ...
        const models2 = await dbCollection.loadDocuments();
        expect(models2.map((model: any) => ({ key: model.key, value: model.value })).sort())
            .to.deep.equal([{ key: 'key1', value: 'newValue1' }, { key: 'key3', value: 'value3' }, { key: 'key4', value: 'value4' }]);
    ...
    it('should clear all items and save to the database', async function () {
        ...
        const models2 = await dbCollection.loadDocuments();
        expect(models2.map((model: any) => ({ key: model.key, value: model.value })).sort()).to.deep.equal([]);
        dbCollection.clear();
        await dbCollection.save();
    });
    ...
    it('should not save if no changes were made', async function () {
        ...
        const models = await dbCollection.loadDocuments();
        expect(models.map((model: any) => ({ key: model.key, value: model.value })).sort()).to.deep.equal([]);
        dbCollection.clear();
        await dbCollection.save();
    });
    it('should close the database, start it again, and create a new collection, initializing it with previous data', async function () {
        ...
        const dbCollection2 = new DatabaseParodyCollection<string, any>('testmodel');
        const models2 = await dbCollection2.loadDocuments();
        expect(models2.map((model: any) => ({ key: model.key, value: model.value })).sort()).to.deep.equal([{ key: 'key1', value: 'value1' }, { key: 'key2', value: 'value2' }]);

        //get collection values and check that its empty
        expect(dbCollection2.map((v,k) => ({ key: k, value: k })).sort()).to.deep.equal([]);
        await dbCollection2.init();
        
        //get collection values and check that its not empty
        expect(dbCollection2.map((v,k) => ({key: k, value: v}))).to.deep.equal([{ key: 'key1', value: 'value1' }, { key: 'key2', value: 'value2' }]);
    ...
    it('should handle duplicate keys', async function () {
        ...
        const models = await dbCollection.loadDocuments();
        expect(models.map((model: any) => ({ key: model.key, value: model.value })).sort()).to.deep.equal([{ key: 'key1', value: 'value2' }]);
        dbCollection.clear();
        await dbCollection.save();
    });
    ...
    it('should handle large number of items', async function () {
        ...
        expect(models.map((model: any) => ({ key: model.key, value: model.value })).sort()).to.deep.equal(expectedModels);
        dbCollection.clear();
        await dbCollection.save();
    });
