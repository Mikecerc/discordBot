...
export default class DynamicEventHandler {
    //events
    private events: DatabaseParodyCollection<
        string,
        IDynamicallyRegisteredEvent
    >;
    constructor() {
        this.events = new DatabaseParodyCollection<
            string,
            IDynamicallyRegisteredEvent
        >("dynamicEvents");
        this.events.init();
    ...
    public async registerEvent(
        event: dynamicEventTypes,
        callback: Function,
        options: { expiryTime?: Date; data: any },
    ): Promise<void> {
        const guid = await this.generateGUID();
        event.setCustomId(guid);
        const initTimeStamp = new Date();
        //default time is 7 days
        const expiryTimeStamp = options.expiryTime
            ? options.expiryTime
            : new Date(initTimeStamp.getTime() + 7 * 24 * 60 * 60 * 1000);
    ...
    public async deleteExpiredEvents() {
        const expiredEvents = this.events.filter(
            (event) => event.expiry < new Date(),
        );
